...
This directory contains MIT-licensed scalar kernels and artifact tooling written
...
remain in their registered GGML host allocation. Both scalar single-tile
kernels are correctness and lifetime tests, not optimized LLM kernels.
...
The Q4_0 kernel decodes the FP16 scale and packed `q - 8` values on-tile without
a dequantized weight copy. The next performance step is AIE2P packed-int4
vector unpack, multi-tile row distribution, and vector/F32 accumulation.
