...
Status: LEVEL 3 - READY FOR MULTI-HOST / HUMAN FINALIZATION for the documented Linux/AIE2P scope. Final local hardening gates pass at source e616bc3 on upstream 3ad1ba7. Not an upstream-quality candidate (4), updated 2026-09-06.
This document records what was inspected and physically exercised; it is not a claim of production-ready XDNA decode.

## Linux/AIE2P production-hardening campaign (2026-09-06)

This campaign preserves published `25528423a178beb3efb45948546dec49a0c280f6` and first rebases onto upstream `73a43d1f69345aee8bb186ef4b3172cef892f2e5`, including its CUDA race correction in the HIP rebuild. Core repair checkpoint: `46d62345900fd8f2372069b144d9dbf8f622ae2f`; hardening checkpoint: `f5ab96a2b13bdbdfcff9736d15549903d252205b`; failure-fixture checkpoint: `6bf091011f63f484198533504ce9418ec2aaffc6`. Callback/packed-Q1 repairs are published through `2e156e44234732c92e454ab532215f75a93edf80`, scale-flags test correction through `7513030cc7780149697a6918ef1652269229ddf9`, and subsequent scheduler/alias repairs through `9235eae3e7ea2dfddb54b8b1ee78ad562643f4fd`. Final synchronized, published and validated source is `e616bc3cc274e8a020e153b9674d03c94fb6789f`, based on upstream `3ad1ba7336986d98592d3e28cafd1a406715351f`; the final checkpoint changes this documentation only. Historical sections retain their original evidence boundaries.

The selective ACCEL architecture remains `GGML -> xdna_problem -> AOT specialization -> XRT`, with capability separate from preference. No model-name dispatch, unused matcher, extra IR, runtime compiler/Python dependency or kernel family was added. Immutable leaf weights retain system-backed registration/view reuse; trainable weights and aliases with unproved immutability use native-format per-call staging. Direct-PARAM and PARAM-view tests now pass with the repair described below.

The runtime no longer assumes a throwing `start()` means nothing was submitted. It requires an explicitly quiescent actual packet state before releasing the run or borrowed storage; unprovable states/query failures fail-stop. Exact installed/candidate XRT resets NEW before submission. The retained old plugin fails the synthetic NEW-state regression by returning; the repair terminates with the required diagnostic. This is policy-fix evidence, not an induced hardware fault.

Independent core review repaired `LLAMA_USE_SYSTEM_GGML=ON`'s accidental XDNA-header dependency: the same upstream install fails before, then compiles/links and passes CPU PARAM after repair without bundled-header mixing. Shared PARAM identity remains preserved: copying can redirect an in-place optimizer to temporary storage, and a `!PARAM` mutation fails the new GPU/IGPU host alias regression. Retained observer, CPU_Mapped, shared-host and transient-copy surfaces are classified generic corrections/minimal hooks. Owner free remains serialized against use/teardown; observers are not lifetime pins.

A fresh CPython 3.12.13 environment uses [pinned AOT requirements](../ggml/src/ggml-xdna/kernels/aie2p/requirements-build.txt): MLIR-AIE 1.3.4, Peano `21.0.0.2026062301+cb664e8c`, compiler-wheel hashes and exact dependencies. Seven bundles rebuild with matching declared ABI/metadata headers; fresh-environment build-rule/controller/geometry tests pass 7/7. The Makefile and compute/controller/packaging C++/Python inputs are unchanged from rebased 18d10fb to 9235eae, so scheduler/alias repairs require no new artifact build. Reproduction requires equivalent inventory/ABI/execution, not identical xclbin metadata. The moving-nightly Peano wheel needs retained hash-verified distribution; inference needs none of these build tools.

Installed XRT 2.20.0 (core `021204355eeaa034ff69aae407ace2265adf047a`) and side-by-side official Ryzen AI 1.8 XRT 2.25.37 (core `0026185de81a179dc7d886197d3e35f9a179b4e8`, matched shim `88dda53eee336c998abca38c71d5bc79fb3633b8`) execute on unchanged in-tree Linux `7.0.0-27-generic` amdxdna and firmware 1.1.2.64. This Ubuntu 24.04 bundle on Ubuntu 26.04 is single-host compatibility, not certification. No system package/module/firmware changed; driver replacement lacks passwordless authentication and completed rollback preflight.

Both roots reject the read-only registration probe with `DRM_IOCTL_AMDXDNA_CREATE_BO`, errno 12. Real BF16 1B read-only mmap runs therefore retain CPU_Mapped storage, execute zero XDNA calls and match the 32-token CPU output on both roots. A third, writable 2.25 run matches that output with 994 calls, 32 immutable views, 962 hits, one-time 1 GiB weight synchronization and zero immutable copies. This validates fail-closed mmap fallback, not working read-only registration.

Portable [package/failure runners](../ggml/src/ggml-xdna/ci/README.md) need no machine-specific paths/system installs. Both SDKs pass static/dynamic/disabled package relocation, consumer-root preservation and missing-SDK rejection, without device initialization. Current/no-XRT/ASan+UBSan/TSAN each pass four scoped host tests. CPU/XDNA/HIP pass all seven immutable variants; XDNA records fourteen changing-input calls, seven views/hits/syncs and zero copies. This is not an all-target build pass: upstream's unrelated server-disabled test-chat/mtmd coupling remains separately attributed.

The extended failure runner passes thirteen cases per root (26 total): post-completion exceptions and synthetic terminal ERROR/ABORT/TIMEOUT retry; unprovable states/query failures; pre-submit/abort exceptions. Eight deliberately fail-stop per root with exact receipts/diagnostics, not bare signals. The three terminal cases were added after publication. Category B/C/pre-submit tests do not establish in-flight recovery; pre-submit still initializes a device.

A real unchanged-kernel probe observes successful start and actual NEW before real abort on both roots. Abort throws `qds_device::wait() unexpected command state`; the original completes, but XRT's second internal abort command remains unproved quiescent. The corrected probe terminates before BO cleanup/retry. Neither AIE-cycle activity nor cancellation/recovery is proved; the old drain-original-and-return probe is not safe recovery evidence. Leaking/disabling XDNA cannot retain independently owned GGML pages, so unprovable quiescence requires fail-stop under the borrowing contract and OS/driver teardown guarantees.

Separate fresh processes on each root complete 10001 changing-input, CPU-exact BF16 backend calls with bounds guards, one root/view, 10000 hits, one weight sync, zero immutable copies, owner destruction while the backend remains alive, and clean teardown. Each GGML destination is poisoned; the device output BO is not. Alternating expected values and per-call comparison detect immediate stale output without claiming stronger device-BO poisoning or recovery of a failed device.

The f5ab96a, post-callback 7513030, repaired-source 9235eae and final upstream-base e616bc3 real-GGUF matrices each pass all five configurations (CPU, forced selective XDNA, HIP, forced HIP+XDNA, ordinary HIP-first+XDNA) for all five models, with identical stdout hashes between checkpoints:

| Model | Modes passed | XDNA calls in each forced XDNA/hybrid case | Immutable views / reuse hits |
| --- | --- | ---: | ---: |
| stories15M Q4_0 | 5/5 | 744 | 24 / 720 |
| Llama 3.2 1B BF16 | 5/5 | 994 | 32 / 962 |
| Llama 3.2 3B BF16 | 5/5 | 1738 | 56 / 1682 |
| Qwen3.5 4B Q8_0 | 5/5 | 1984 | 64 / 1920 |
| Qwen3.5 9B Q4_K_M | 5/5 | 0, expected unsupported fallback | 0 / 0 |

All five generated outputs agree byte-for-byte within each model; exact hashes, placement identities, first real dispatches, unchanged inputs and successful terminal receipts are retained. Ordinary HIP-first records zero XDNA calls for every model; all cases have zero immutable copies. Scheduler debug identities include planning graphs and truncate names, while aggregate counters establish actual XDNA call counts. This remains 32-token smoke evidence, not every intermediate/logit proof. Global swap occurred even in the final e616bc3 matrix (3B CPU: 4503 pages in, 161489 out), so no timings are performance-valid. Every production descriptor remains `prefer_for_offload=false`.

Physical instrumentation exposed inherited generic defects, now repaired independently of XDNA: seven CPU and 51 base callback casts use private exact-signature adapters (`7d903f4451171277af55b979bc4eb060ac5b940b`), and AVX2 Q1_0 loads its packed field via memcpy into aligned local words (`2e156e4`). Thirteen independent old-library UBSan probes fail before and pass after; complete quantization testing passes, including every generic callback. Public ABI and numerical algorithms are unchanged. Historical [issue #6202](https://github.com/ggml-org/llama.cpp/issues/6202) reports the same callback defect class but was closed stale, not fixed. These are class-A generic repairs, not XDNA kernel changes.

The corrected fully ASan/UBSan-instrumented GGML/CPU/XDNA runtime and runners now pass on both XRT roots: each completes 10001 changing-input CPU-exact persistent calls with guards, registration reuse, zero immutable copies and owner-free/backend teardown; each also passes seven immutable variants plus mutable BF16 (8/8, 16 calls, seven immutable views/hits, two intentional mutable copies totaling 331776 bytes). Leak detection and halt-on-error remain enabled; online symbolization alone is disabled. XRT and the kernel driver are uninstrumented. Instrumented CPU upscale/interpolation controls pass 22/22 after a test-only uint32 mode/flags correction using existing APIs.

Failed intermediates remain explicit: the initial CPU callback report preceded any XDNA compute, followed by symbolizer blocking/timeout 124, not an NPU hang. A later suite passed 10001 persistent calls but stopped on invalid scale-enum construction before its selected XDNA op tests; this motivated the test-only correction. The final `runtime-asan-acceptance-fixed` suite exits 0 with unchanged hashes. At published 7513030, ordinary host/HIP builds and all four configurations' four registered tests plus independently executed full quantization pass with source hashes unchanged. `post-callback-physical` completes CPU/HIP/XDNA seven-variant controls and thirteen failure cases per SDK; `gguf-matrix-post-callback` completes all 25 cases. Later source receipts remain distinguished below.

A subsequent audit physically confirms PARAM-view misclassification at published 7513030: CPU passes 1/1, while XDNA fails 0/1 with error 3.997717338 after negating BF16 weights, reusing the registration without a second weight synchronization/copy. The fixture's mutation/reference oracle was independently reviewed. GGML views can lose PARAM flags, including flattened intermediate aliases; the registry repair conservatively stages aliases rather than attempting to reconstruct lost trainability. Its 42 host classifications and repaired-device PARAM-view case pass ASan/UBSan. The earlier loader-only exit 127 remains preserved separately, not hardware evidence.

The shared-input scheduler concern is confirmed by the corrected host mock: mixed copied/shared inputs produce -99 instead of 5 before producer completion (`shared-async-before-v3`); independent comparison finds the entire pre-fix 10010-byte `compute_splits` function identical to upstream 73a43d1. The private per-split source-backend mask records shared dependencies, follows pure metadata `src[0]` edges to the actual writer and synchronizes those producers before copies/consumer execution. It adds no public API; four mocks cover shared/copied companions and direct/view sources. Independent review is in `SHARED-DEPENDENCY-REVIEW.md`. Both repairs are published: scheduler e2d308d, alias 9235eae.

Final repaired-source `edge-final-host` passes four registered tests plus full quantization in each of four configurations (20 checks), with complete CPU/HIP builds. `runtime-asan-acceptance-edge` passes both SDKs: nine cases each (seven immutable, PARAM leaf, PARAM view), 18 calls, seven immutable views/hits, four intentional staging copies totaling 663552 bytes; each SDK also completes 10001 CPU-exact persistent calls with guarded owner-free/teardown. CPU upscale controls pass 22/22; ASan/UBSan/leak checks remain enabled. `edge-final-physical` passes seven variants on CPU/HIP/XDNA, both HIP PARAM cases and thirteen failure cases per SDK. Both SDK package repeats complete/0 in `post-callback-packages`, including static/dynamic/disabled relocation and required-SDK rejection, without device initialization. The repaired-source `gguf-matrix-edge-final` completes 25/25 with no exceptions. All three `mmap-post-callback-*` repeats pass: both read-only cases safely fall back, and writable XRT 2.25 retains 994 calls, 32 views, 962 hits and zero immutable copies.

After these completed runs, the final upstream fetch advances to `3ad1ba7336986d98592d3e28cafd1a406715351f`: OpenCL packed-weight eligibility and Spark2.5 model support, with no scheduler/buffer/XDNA/AOT changes. Rebase preserves all 62 development commits as patch-equivalent changes and publishes e616bc3 using an explicit lease against 9235eae. Both CPU and HIP builds regenerate their source inventory, including the new Spark source. All seven AOT compute/controller/packaging inputs remain unchanged; the fresh bundles are physically revalidated without an unnecessary rebuild.

Final new-base receipts all complete with exit 0: `final-upstream-host` passes 20 checks across the four host configurations and rebuilds CPU/HIP; `runtime-asan-final-upstream` passes 10001 persistent calls and all nine op cases per SDK, with the 22 CPU interpolation controls run once outside the SDK loop; `final-upstream-physical` repeats CPU/HIP/XDNA immutable controls, HIP PARAM leaf/view and 26 attributed failure cases. `gguf-matrix-final-upstream` passes 25/25 with no audit exceptions, and all three `mmap-final-upstream-*` cases retain the documented fallback/writable behavior. Sanitizer, failure-injection, output-poisoning and performance boundaries above still apply.

`final-upstream-packages` rebuilds and relocates static/dynamic/disabled packages for both SDKs and passes independent consumers and required-SDK negative tests. The additional `final-upstream-packaged-device-v2` physically executes nine op cases per SDK through the relocated dynamic packages; actual loader initialization traces select the relocated GGML libraries and corresponding XRT core/shim. Static packages have consumer/API coverage, not a separate static physical inference claim. The first cache-local package execution wrapper incorrectly overwrote its library path: the 2.25 case loaded only CPU and was rejected for missing XDNA coverage. Its exit-1 receipt remains preserved as a harness failure, not numerical or hardware failure; the corrected wrapper has explicit loader evidence. The resumed final suite completes successfully.

Fork CI is independently attributed at published e616bc3: CANN run `34044011558` has zero jobs and schema annotation `(Line: 36, Col: 6): Unexpected value ''`, inherited from upstream's empty/commented-out `jobs:`. Three Python checks newly triggered by the upstream conversion changes are queued with no assigned runner or executed steps. The repository snapshot has seven queued runs and no registered runners, plus fifteen failures: twelve CANN schema rejections and three older master failures. Older master WebGPU/Apple runtime failures are real source/test failures, while Docker dependency acquisition and missing runners are infrastructure; none establishes an XDNA regression or CI-green branch. No unrelated CANN/workflow changes were made.

The factual handoff and final validation ledger are `HARDENING-HANDOFF.md` and `FINAL-UPSTREAM-VALIDATION.md` under `/home/hamza-usta/.cache/llama-xdna-hardening-20260906-Bc4Pzv`; raw and failed-run evidence remains intact. **LEVEL 3 - READY FOR MULTI-HOST / HUMAN FINALIZATION**, with completed local gates on current upstream base 3ad1ba7 and no known supported-path source defect intentionally waived. Level 4 is not claimed. External gates remain human acceptance of generic hooks/copy policy, a second supported Linux host and broader driver evidence, vendor-defined failed-reset/hidden-abort quiescence, pinned wheel retention and physical CI. Positive read-only mmap remains unsupported on this tested stack; a newer kernel-driver test needs privileged operation and a completed rollback plan. No profitability, broader OS/AIE2 support or upstream submission is implied.
